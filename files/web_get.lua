...
local function get(t, cb)
 ...
 con:on("disconnection", function(con)file.close()cb(false)con = nil end)
...
return function (t, cb)
 ...
 if t.init =="upload" then
   ...
   if f then
    ...
    if ok then t.file=j upload(t,cb) else cb(false) end
   end
   end)
 else
  get(t,function(d) temp(d)cb(d) end)
 end
end
